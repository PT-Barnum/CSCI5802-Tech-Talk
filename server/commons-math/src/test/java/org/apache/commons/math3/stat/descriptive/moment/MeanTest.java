...
public class MeanTest extends StorelessUnivariateStatisticAbstractTest{
    ...
    @Test
    public void testMeanConstructor() {
        Mean mean = new Mean();
        Assert.assertEquals(mean, new Mean(mean));
    }

    @Test
    public void testCopyNullArgumentExceptionDest() {
        Mean mean = new Mean();
        Boolean nullArgumentExceptionThrown = false;

        try {
            Mean.copy(mean, null);
        }catch (Exception e) {
            if(e instanceof NullArgumentException) {
                nullArgumentExceptionThrown = true;
            }
        }
        Assert.assertTrue(nullArgumentExceptionThrown);

    }

    @Test
    public void testCopyNullArgumentExceptionSource() {
        Mean mean = new Mean();
        Boolean nullArgumentExceptionThrown = false;
